...
public class Spanny extends DraweeSpanStringBuilder {
    ...
    public Spanny append(CharSequence text, ArrayList<Object> spans) {
        append(text);
        for (Object span : spans) {
            setSpan(span, length() - text.length(), length());
        }
    ...
    public Spanny append(Context context, @DrawableRes int drawRes,
                         @DimenRes int width, @DimenRes int height) {
        Drawable drawable = ContextCompat.getDrawable(context, drawRes);
        if (drawable != null) {
    ...
    public Spanny appendDrawableInPix(Context context, @DrawableRes int drawRes,
    ...
    public Spanny append(Context context, String remotePicUrl, int defaultDrawable, int width, int height) {
        GenericDraweeHierarchy medalHierarchy = new GenericDraweeHierarchyBuilder(context.getResources())
                ...
                .build();
        DraweeHolder<GenericDraweeHierarchy> medalDraweeHolder = DraweeHolder.create(medalHierarchy, context);
        DraweeController controller = Fresco.newDraweeControllerBuilder()
                .setUri(remotePicUrl).build();
        medalDraweeHolder.setController(controller);
        int tmpLength = length();
        append(" ");
        setImageSpan(medalDraweeHolder, tmpLength, tmpLength, width
                , height, true, BetterImageSpan.ALIGN_CENTER);
        return this;
    }
